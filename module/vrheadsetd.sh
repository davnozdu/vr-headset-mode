...
while true; do
    ...
    if [ "$present" = yes ] && [ "$mode" = headset ]; then
        if [ "$ready" = no ]; then
            ...
            # Повторяем молча. Дисплей возвращается сам после выхода телефона
            # из сна, а команда идемпотентна и дёшева — проще применять её
            # регулярно, чем искать надёжный признак "он снова включён".
            hide_quiet
        fi
    fi
